    ...
    class MethodDropdown(discord.ui.Select):
        ...
        async def callback(self, interaction: discord.Interaction):
            ...
            try:
                ...
                user_data = {
                    "client requesting": client_id,
                    "list of books": view.selected_book,
                    "amount requested": f"${amount:.2f}",
                    "method": selected_method,
                    "deposited initiated": date
                }
                headers = sheet.row_values(1)
                headers_lower = [header.lower() for header in headers]

                val = [user_data.get(header, "") for header in headers_lower]
